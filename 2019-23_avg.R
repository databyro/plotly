...
Sys.setenv("plotly_username"="julius.lauri.lehtinen")
Sys.setenv("plotly_api_key"="w1bGHsPL3zejCn7XCH0o")
...
X2019_23 <- read_excel("C:/Users/Julius/Setit/Databyro/excel ja R/excelit/2019-23.xlsx")
data = X2019_23
date = X2019_23$date
kok = X2019_23$Kok
kesk = X2019_23$Kesk
...
fichart <- plot_ly(data, x = ~as.Date(date)) %>%
  ...
  layout(title= list(text="Puolueiden kannatus", x=0), titlefont=list(size=24,family="Philosopher"), annotations = list(text = "Databyro.fi",
      font = list(size = 12),showarrow = FALSE, xref = 'paper', x =1, yref = 'paper', y = 0.95), margin = list(l = 10, r = 10, t = 50, b = 10),
      ...
      hovermode = 'compare', legend = list(y = -0.07, x = 0, orientation = 'h')) %>%
  
  config(displayModeBar = F)
fichart
api_create(fichart, filename = "r-fi-polls3")
...
govtchart <- plot_ly(data, x = ~as.Date(date)) %>%
  ...
  layout(title= list(text="Hallituspuolueiden ja oppositiopuolueiden<br>yhteenlasketut kannatukset", x=0), titlefont=list(size=24,family="Philosopher"), annotations = list(text = "Databyro.fi",
      font = list(size = 12),showarrow = FALSE, xref = 'paper', x =1, yref = 'paper', y = 0.95), margin = list(l = 10, r = 10, t = 100, b = 10),
      xaxis=list(fixedrange=TRUE, tickformat = "%d.%m.%Y", title = "",titlefont = list(family="Philosopher"),
      range=c(as.numeric(as.Date("2019-04-14")) * 24 * 60 * 60 * 1000, as.numeric(as.Date("2021-04-15")) * 24 * 60 * 60 * 1000)),
  ...
  config(displayModeBar = F)
govtchart
api_create(govtchart, filename = "r-fi-polls4")